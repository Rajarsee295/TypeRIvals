   ...
   if (/^[a-zA-Z]$/.test(event.key)) {
      if (event.key != str.charAt(counter)) {
         ...
         if (str.charAt(counter) == ' ') {
            ...
            cursor[0].style.left = document.getElementById(`extra${extra_letters - 1}`).getBoundingClientRect().left + 20 + "px";
            raw++;
         }
         else {
...
function wpm_calculator(){
   ...
   localStorage.setItem("time_woohoo",timer_copy);
   localStorage.setItem("raw",(parseInt(raw/(5*timer_copy/60))));
